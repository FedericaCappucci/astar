...
Node* generateSuccessors(Node current)
{
 	int dy[8] = {0, 1, 0, -1,1,-1,-1,1}; //row
        int dx[8] = {-1, 0, 1, 0,1,-1,1,-1}; //col
        Node successori[8];
        for(int pind = 0; pind<8; pind++) // position possible 
	{
		Node succ;
		succ.Ncol=current.Ncol+dx[pind];
		succ.Nrow=current.Ncol+dy[pind];
		successori[pind]=succ;
	}
return successori;        
}
int a_star(Node *start, Node *destination)
{
   ...
   cout <<"dest" <<destination->Ncol;
    ...
    while(!openList.empty()) //per parallelizzazione meglio for
    {
        ...
        Node *successori = generateSuccessors(current);
        	...
        	for(int pind = 0; pind<8; pind++) // position possible 
		{
			int dy[8] = {0, 1, 0, -1,1,-1,-1,1}; //row
				...
				//vc = current.Ncol + dx[pind]; //col
		    	   ...
		    	   Node app= successori[pind];
			    if(isValid(app.Nrow,app.Ncol))
			{
			    Node successor;
			    ...
			    {
			    	cout<<"arrivo\n";
			    	//non più aggiorna, prima controlla lista
