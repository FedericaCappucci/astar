  ...
  // cout <<"dest" <<destination->Ncol;
    if(counterNeg!=0){
    	...
    	#pragma omp parallel
    	{
			...
			cout<<"\n Time: "<<end-beg << " the best path was the one of " << path_array[0].numThread;
			for(int i=0;i<(sizeof(path_array)/sizeof(path_array[0]));i++)
			{
				cout<< "\n thread : " << path_array[i].numThread <<"has found path with cost: "<< path_array[i].cost;
			}
	}//if 
